...
\paragraph{Speed.}
\figureref{fig:perf-scaling} shows the computational scaling of the three methods. The standard GP's cubic scaling with observations makes it the slowest. The EnKF is significantly faster, showcasing the benefit of the ensemble approach. The LETKF is faster still, especially as the state dimension grows, because localization breaks the global problem into many smaller, independent calculations. This demonstrates the profound efficiency gains possible by adopting mature DA techniques.
