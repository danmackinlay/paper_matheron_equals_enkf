...
\begin{figure}[t]
  \floatconts
    {fig:timing-curves} % label
    {\caption{Timing comparison showing fit (solid) vs predict (dashed) times. Left: Scaling vs. number of observations ($m$). Right: Scaling vs. state dimension ($d$). GP prediction scales quadratically while EnKF/LETKF prediction remains effectively flat, demonstrating the computational advantages of ensemble methods.}} % caption
