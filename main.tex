...
Combining these two gives the effective covariance in the observation space.\footnote{The EnKF extends to nonlinear observation operators, though Gaussian assumptions no longer hold exactly~\cite{Evensen2009Data}.}
...
Since our focus here is to demonstrate simplicity of implementation, we use the off-the shelf solver DAPPER~\cite{Raanes2024DAPPER} which allows us to demonstrate the parallels.
We note however that DAPPER is not optimised for high efficiency posterior sampling and we do not expect it to achieve maximally efficient scaling with problem size; in particular its scaling with state dimension seems to be sub-optimal.
Code is avaialble in supplemental
...
\paragraph{Accuracy.}
\figureref{fig:posterior-samples} shows 100 draws from each posterior at
...
\paragraph{Speed.}
\figureref{fig:perf-scaling} shows scaling behavior across both observations and state dimensions.
The EnKF maintains computational efficiency in both regimes,
scaling as $\mathcal{O}(dm^2)$ with observations and $\mathcal{O}(d)$ with state dimension,
while GP regression scales as $\mathcal{O}(m^3)$ and requires full kernel matrix operations.

\begin{figure}[htbp]
  \floatconts
    {fig:posterior-samples} % label
    {\caption{Posterior draws at $d=1000$.
             Means overlap to eye and to $<\!0.03$ RMSE.}} % caption
    {\centering\includegraphics[width=\linewidth]{posterior_samples.pdf}}
\end{figure}

\begin{figure}[t]
  \floatconts
    {fig:perf-scaling} % label
    {\caption{Left: Wall-clock time vs. number of observations ($m$) for a fixed state dimension ($d=2,000$). Right: Wall-clock time vs. state dimension ($d$) for a fixed number of observations ($m=2,000$).}} % caption
    {\centering\includegraphics[width=\linewidth]{perf_scaling.pdf}}
\end{figure}
...
This is achievable with  with \emph{zero}
algebraic changes to the off-the-shelf geophysical code.
The various conveniences built into the geophysical toolkits such as covariance localisation etc are potentially useful to the GP inference community, as is the prospect of improved scaling with number of observations and dimensionality of state.
