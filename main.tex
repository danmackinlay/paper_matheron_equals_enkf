...
The computational attractiveness of the Ensemble Kalman Filter lies in the fact that the analysis update can be performed using only empirical ensemble statistics, thereby avoiding the direct computation and inversion of high-dimensional covariance matrices.
...
\end{align}
where \(\gamma^2 = \upsilon^2 + \rho^2\) aggregates the regularization of the empirical observation covariance and the observation noise covariance.

Let us now break down the computational cost of this update:

\begin{enumerate}
    \item \textbf{Forming the Empirical Covariance:}
    Computing \(\mmdev{Y}^\top \mmdev{Y}\) involves multiplying a \(D_{\vrv{y}}\times N\) matrix \(\mmdev{Y}\) with its transpose (an \(N\times D_{\vrv{y}}\) matrix). This step requires \(O(D_{\vrv{y}}N^2)\) operations.

    \item \textbf{Inversion of the \(N\times N\) Matrix:}
    The inversion of \(\mmdev{Y}^\top \mmdev{Y} + \gamma^2\mm{I}\) costs \(O(N^3)\) operations.

    \item \textbf{Matrix Multiplications to Form the Gain:}
    Multiplying \(\mmdev{X}\) (a \(D_{\vrv{x}}\times N\) matrix) with \(\mmdev{Y}^\top\) (an \(N\times D_{\vrv{y}}\) matrix) requires \(O(D_{\vrv{x}}D_{\vrv{y}}N)\) operations. Additional multiplications involving the inverted \(N\times N\) matrix and \(\mmdev{Y}\) contribute a cost on the order of \(O(D_{\vrv{y}}N^2)\).
\end{enumerate}
...
\]
Since in practical applications the ensemble size \(N\) is typically much smaller than the dimensions \(D_{\vrv{x}}\) and \(D_{\vrv{y}}\), these costs are significantly lower than those incurred by the standard Kalman filter, which involves inverting a \(D_{\vrv{y}}\times D_{\vrv{y}}\) matrix at a cost of \(O(D_{\vrv{y}}^3)\).
