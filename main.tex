...
\paragraph{Speed.}
\figureref{fig:posterior-timing} plots wall-clock time with error bars (5 trials each).
...
\begin{figure}[htbp]
  \floatconts
    ...
    {\centering\includegraphics[width=\linewidth]{fig_posterior_samples.pdf}}
\end{figure}

\begin{figure}[htbp]
  \floatconts
    {fig:posterior-timing} % label
    {\caption{Wall-clock to compute one posterior update
             (log–log). Training EnKF scales as $\mathcal O(dm^{2})$, GP as $\mathcal O(m^{3})$. We set $m=d/5$.}} % caption
    {\centering\includegraphics[width=\linewidth]{fig_posterior_timing.pdf}}
\end{figure}
