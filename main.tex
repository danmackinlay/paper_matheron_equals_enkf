...
% ----------------------------------------------------------------
\section{Numerical Illustration:  One–D Kriging Three Ways}
\label{sec:numerical-demo}
% ----------------------------------------------------------------

We re-visit the classic kriging toy problem but now compare three solvers
side-by-side:
...
We test grid sizes $d\in\{500,1000,2000,4000\}$.
\end{quote}

\paragraph{Methods.}
\begin{itemize}[leftmargin=1.8em]
\item \textbf{Woodbury path-wise (analytic).}  Eq.\,(1) with the
      $m\!\times\!m$ Woodbury inversion; cost $\mathcal O(dm^{2})$.
\item \textbf{FilterPy EnKF.}  \texttt{EnsembleKalmanFilter}
      (\texttt{N} = $\max(200,d/10)$) with a $10^{-8}$ nugget,
      multiplicative deflation (0.84) and no code changes.
...
\begin{figure}
  ...
  \caption{Posterior draws at $d=1000$.
           Blue = Woodbury, green = EnKF, orange = GP.
...
\begin{figure}
  ...
  \caption{Wall-clock to compute one posterior update
           (log–log).  EnKF scales linearly, GP cubically,
           Woodbury as $\mathcal O(dm^{2})$.}
  \label{fig:posterior-timing}
\end{figure}

\paragraph{Take-away.}
A commodity EnKF implementation—originally written for atmospheric data
assimilation—delivers path-wise GP draws whose accuracy matches the
analytic update while retaining the $\mathcal O(Nd)$ cost familiar to
the geophysical community.  The result is an order-of-magnitude speed-up
