...
%\nb{this is still not QUITE right - what if there are other observed conditioning vars?}
...
So that we may solve such systems computationally, we operate not directly in the continuously-indexed function spaces, but on discrete representations.
...
\end{align*}
% We make the projection explicit only when it cannot be inferred from context.
...
%\russ{Do we use this inverse? (beyond the composition with the projection to update in discrete space)}
\begin{align*}
    \proj_{\Phi}^{-1}\left\{\begin{bmatrix}\tilde{f}_1&\cdots&\tilde{f}_{D_{\vrv{z}}}\end{bmatrix}^{\top}\right\} = \sum_{i=1}^{D_{\vrv{z}}} \tilde{f}_i\phi_i.
\end{align*}

By some abuse of notation we use the same representation for the forward operator \(\op{P}\) and its discretised version, writing \(\op{P}(\statest, \forcingst)\) to mean \(\proj_{\Phi}\op{P}(\proj_{\Phi}^{-1}\statest, \proj_{\set{Y}}^{-1}\forcingst)\) where now \(\op{P}:\mathbb{R}^{D_{\vrv{z}}}\times \mathbb{R}^{|\set{Y}|} \to \mathbb{R}^{D_{\vrv{z}}}\).
% \nb{still shonky; in fact we \emph{do} take pointwise observations in the forward projection step}
% \russ{Is it important for us to distinguish projections from functions themselves? It seems as though the computational methods deal with projections or evaluations. There is a lot of terminology (projections, inverses, and their compositions) introduced and I wonder whether it is necessary?}
% \danm{I also wonder that. I don't think the continuity stuff is necessarily helping us much; OTOH it only makes esne to folve problems with thousands of correlated dimension ebcause of PDEs. I wonder if we can move straight to the discretised version and punt the PDE interpreation to the appendix?}
% We note in passing that due to this representation, there is nothing specifically  spatial about these fields, and thus it is a trivial extension of our approach to infer scalar-valued parameters or  simultaneously infer spatial, function-valued and scalar-valued parameters.
% A scalar parameter may be treated a trivial field with a trivial basis.

Throughout this work, our examples use the top-hat finite element basis, a set of piecewise constant, top-hat functions partitioning the space, centred on a square lattice of \(S\times S\) points partitioning the unit square, e.g. in two dimensions the index set is \((i,j) \in [1,\dots,K]\times [1,\dots,K]\) with %\russ{I saw something at NeurIPS this year about fast inference with top hat basis in some restricted setting --- did you see it?}
\begin{align*}
...
% Thus, our methods must be amenable to large vectors.
%In statistical settings, we do not have access to the infinitude of values that defines a function a continuous index, but rather we observe some finite-dimensional projection of the states.
%The elements of ${\statest}$, through finite-dimensional projections \(\seenstate{}: \statesp \to \mathbb{R}^{d_\state},\seeninp{}: \forcingsp \to \mathbb{R}^{d_\forcing}\).
%Of particular interest here is the special projection \(\proj_{\vv{S}}:\space\) which maps a function to a vector of evaluations of that function at a finite vector of test sites \footnote{more general projection, e.g. onto a functional basis, is similar.} \(S =[s_1\,s_2\,\cdots,s_{|S|}] \subseteq \set{S}\).%\russ{Previously we said $\set{S}$ was the unit square (an uncountable set?), so I guess we should write$\subset$ instead of $\subseteq$? }\danm{haha yeah. there is still something shonky about the discretisation in this whole thing}
%\footnote{To keep notation uncluttered we assume hereafter that the estimand is a scalar-valued field, but extension to vector-valued field is straightforward, by e.g. re-stacking vectors of vector-valued evaluations into a single vector.}
%Specifically,
%\begin{align*}
%    \proj_{\vv{S}}f: &f \mapsto [f(s_i) ; i=1,\cdots,|S|].
%\end{align*}
%We define an an analogous projection \(\proj_{\vv{X}}\) for observations of forward predictions, where it is not necessary that \(S=X\).\nb{cute, but I don't think we need this any longer?}
%For brevity we refer to a projected variable by decorating it with tildes, e.g. \(\state_{t}:=\proj_{\vv{X}}\state_{t}\).
% Observations of the system may also suffer from some additional noise, $\rv{e}_i$, so that the resulting time series might be ${\state}_{1:t-1} = (\seenstate{}\statest_0 + \rv{e}_0, \cdots, \seenstate{}\statest_{t} + \rv{e}_{t})$, where $\statest_i$ is an additive random variable with the same finite dimension as $\seenstate{}\statest_i$.
% As a statistical problem, forward prediction becomes the task of estimating the conditional distribution $\state_{T} | {\state}_{1:t-1}$. 
% Specifically,  \(\statest_{1}\simeq \state_{1}\gvn \forcing, \statest_{2}\simeq \state_{2}\gvn \state_{1},\forcing  \cdots,\statest_{T}\simeq \state_{1}\cdots \state_{T-1},\forcing \).
...
%\nb{project}
%\nb{Likewise, I thought sans-serif fonts were for observations?  here your font implies these are random variables}.
%\russ{Wouldn't we observe $\state_t$?}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1] % The argument is the first line number
            \Require{Prior $p(\forcingst)$, Forward model $\op{P}(\cdot \,, \cdot)$, Ensemble size $N \geq 1$, Initial observation $\statest_0$, Observation stream $\statest_t$ for $t=1, \ldots, T$}
        \Procedure{\meth{} }{}
          \State $t \gets 0$
          \State $\mm{U}_t = [\forcingst_0^{(1)}, \ldots \forcingst_0^{(N)}]\gets \text{\sc{Sample}}\big(p(\forcingst), N\big)$
          \Comment{Sample $N$ times from the prior and store in a $D_{\forcing} \times N$ matrix}
            \While{\(t < T\)}
                \State $t \gets t + 1$
                \State $\mm{Z}_t = [\statest_t^{(1)}, \ldots, \statest_t^{(N)}] \gets \op{P}(\statest_{t-1}, \forcingst_{t-1})$ \Comment{Update the predictive ensemble}
                \State $\breve{\mm{Z}}_t \gets \text{\sc{ComputeDeviation}}(\mm{Z}_t)$ \Comment{Compute deviation matrices}
                \State $\breve{\mm{U}}_{t-1} \gets \text{\sc{ComputeDeviation}}(\mm{U}_{t-1})$ \Comment{Compute deviation matrices}
                \State $\statest_t \gets \text{\sc{MakeObservation}}(t)$ \Comment{Make an observation from the stream}
    ...
    \right)$ \Comment{Update parameter ensemble}
